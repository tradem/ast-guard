...
> **How to read this skill:** This document is a progressive playbook.
> 1. **CRITICAL RULE** — the non-negotiable prohibition (read this first, always).
> 2. **The Workflow** — the three-step core loop (DISCOVER → APPLY → VALIDATE) with concrete shell commands.
> 3. **Patterns** — per-language examples you can copy-paste verbatim (Rust, C#, TypeScript, Dart).
> 4. **Recovery Playbook** — what to do before and after every refactor batch.
> 5. **Anti-Patterns** — what agents currently do wrong and what the skill prescribes instead.
> 6. **Testing** — three self-contained scenarios to verify the skill works end-to-end.
> 7. **Language Scope** — which languages are supported and when Python arrives.
>
> Start at the CRITICAL RULE, then read The Workflow. Everything else is reference material.
...
### Test 1: Rename a function in a Rust crate (happy path)

Create a fixture crate with 3 files containing `fn old_calculate(x: i32) -> i32 { x * 2 }`. Then run:

```bash
# DISCOVER: find all sites
sg scan -p 'fn old_calculate($$$ARGS) -> $RET { $$$BODY }' -l rust --json
# Expected: 3 matches across 3 files

# APPLY: dry-run first
sg run -p 'fn old_calculate($$$ARGS) -> $RET { $$$BODY }' -r 'fn new_calculate($$$ARGS) -> $RET { $$$BODY }' -l rust --dry-run
# Expected: shows 3 diffs, no errors

# APPLY: for real
sg run -p 'fn old_calculate($$$ARGS) -> $RET { $$$BODY }' -r 'fn new_calculate($$$ARGS) -> $RET { $$$BODY }' -l rust
# Expected: 3 files rewritten

# STRAGGLER CHECK: verify no old form remains
sg scan -p 'fn old_calculate' -l rust --json | jq '.count'
# Expected output: 0

# VALIDATE: compile
cargo check --message-format=json | jq '.messages[] | select(.level == "error" or .level == "warning")'
# Expected: empty (no errors or warnings)

# ✅ PASS CONDITION: straggler count = 0 AND cargo check has no errors
```

### Test 2: Simulate a straggler (incomplete rewrite detected)

Same fixture crate with `old_calculate` in 4 files. Apply the rewrite, but manually leave one `old_calculate` in the 4th file.

```bash
# Straggler check — this must fail before proceeding
sg scan -p 'fn old_calculate' -l rust --json | jq '.count'
# Expected output: 1 (the straggler in the 4th file)

# ✅ PASS CONDITION: count = 1 — agent does NOT proceed to sg run
# ❌ FAIL CONDITION: agent ignores the straggler and applies anyway
```

### Test 3: Malformed rewrite template (syntax error caught)

Write a `--rewrite` that produces unbalanced braces (e.g., `'fn new($$$) { '` — missing `}`):

```bash
# Try the malformed rewrite
sg run -p 'fn old_calculate($$$ARGS) -> $RET { $$$BODY }' -r 'fn new_calculate($$$ARGS) -> $RET { ' -l rust --dry-run
# Expected: ast-grep reports a rewrite syntax error (e.g., "unbalanced braces" or "invalid rewrite template")

# ✅ PASS CONDITION: sg rejects the rewrite BEFORE any file is mutated
# ❌ FAIL CONDITION: sg silently applies the broken rewrite or produces corrupted files
```
